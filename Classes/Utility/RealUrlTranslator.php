...
class RealUrlTranslator
{
    ...
    protected function decodeAlias($parameters, &$ref)
    {
        ...
        $where .= ' AND value_alias = ' . $GLOBALS['TYPO3_DB']->fullQuoteStr($parameters['value'],
    ...
    protected function encodeAlias($parameters, &$ref)
    {
        ...
        if (!empty($table)) {
            ...
            if (empty($configurations[$table])) {
                ...
                if ($this->configuration['debug'] || TYPO3_DLOG) {
                    GeneralUtility::devLog(sprintf('No alias configuration found for table %s, falling back on default (uid)',
                            $table), $this->extKey, 2, $configurations);
                }
                ...
                if (empty($configuration['alias_field'])) {
                    if ($this->configuration['debug'] || TYPO3_DLOG) {
                        GeneralUtility::devLog(sprintf('Undefined alias field for table %1$s, using %2$s instead',
                                $table, $configuration['id_field']), $this->extKey, 2, $configuration);
                    }
        ...
        if (!empty($id) && !empty($table)) {
            ...
            $where = "tablename = " . $GLOBALS['TYPO3_DB']->fullQuoteStr($table,
                            'tx_realurl_uniqalias') . " AND value_id = '" . $id . "'";
            ...
            if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) > 0) { // As alias exists
                ...
                if ($row['expire'] < time() && $row['expire'] > 0) {
                    ...
                    $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_realurl_uniqalias', "uid = '" . $row['uid'] . "'",
                            $fields);
    ...
    protected function checkUniqueAlias($alias, $separator)
    {
        ...
        do {
            $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('value_alias', 'tx_realurl_uniqalias',
                    'value_alias = ' . $GLOBALS['TYPO3_DB']->fullQuoteStr($uniqueAlias, 'tx_realurl_uniqalias'));
            if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) == 0) {
                $hasUniqueAlias = true;
                break;
            } else {
